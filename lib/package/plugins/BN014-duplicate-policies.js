...
const _diffXmlNode = (isTopLevel) => (indent, elt1, elt2) => {
  ...
  if (elt1.nodeType == ELEMENT_NODE) {
    ...
    if (!diff) {
      ...
      diff = children1.length != children2.length;

      if (!diff) {
        debug(`${indent}recurse`);
        diff = !!children1.find((elt, ix1) => {
          const elt2 = children2[ix1];
          return _diffXmlNode(false)(indent + "  ", elt, elt2);
        });
      }
    }
    return diff;
      ...
      try {
        ...
        if (dupesForI.length) {
          ...
          dupesForI.forEach((ix) =>
            _markPolicy(
              policies[ix],
              `Policy ${policies[ix].getName()} is a duplicate of Policy ${policies[i].getName()}.`
            )
          );
          flagged = true;
...
const check = (bundle) => {
  ...
  if (bundle.policies) {
    ...
    if (bundle.policies.length > 1) {
      flagged = _checkForDuplicatePolicies(bundle.policies);
    }
  }
  return flagged;
};
