...
class EndpointChecker {
  ...
  check() {
    ...
    try {
      ...
      if (this.corsPoliciesInBundle) {
        const keys = Object.keys(this.corsPoliciesInBundle);
        ...
        const corsStepsInEndpoint = this.endpoint
          .getSteps()
          .filter((step) => keys.indexOf(step.getName()) >= 0);

        if (this.isProxyEndpoint) {
          if (corsStepsInEndpoint.length > 1) {
            flagged = this._checkStepsForMissingConditions(corsStepsInEndpoint);
          } else {
  ...
  _checkStepsForMissingConditions(steps) {
    const anyStepsLackCondition = steps.filter((step) => {
      if (!step.getCondition()) {
        _markStep(
          step,
          //this.corsPoliciesInBundle[name],
          `There are multiple CORS policies and policy ${step.getName()} is attached to a Step without a Condition.`
        );
        return true;
      }
    });
